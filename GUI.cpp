...
void CGUI::Draw()
{
	...
	if (ShouldDrawCursor)
	{
		...
		for (int i = 0; i < 4; i++)
			Render::Clear(Mouse.x + 2 + i, Mouse.y + 2 + i, 1, 14 - (i * 2), Color(0, 204, 0, 255));
		Render::Clear(Mouse.x + 6, Mouse.y + 6, 1, 8, Color(0, 204, 0, 255));
		Render::Clear(Mouse.x + 7, Mouse.y + 7, 1, 9, Color(0, 204, 0, 255));
		for (int i = 0; i < 4; i++)
			Render::Clear(Mouse.x + 8 + i, Mouse.y + 8 + i, 1, 4 - i, Color(0, 204, 0, 255));
		Render::Clear(Mouse.x + 8, Mouse.y + 14, 1, 4, Color(0, 204, 0, 255));
		Render::Clear(Mouse.x + 9, Mouse.y + 16, 1, 2, Color(0, 204, 0, 255));
	}
}
...
#define UNLEN 256
	...
	//rainbow bar
	Render::Clear(window->m_x + 1, window->m_y + 8, window->m_iWidth - 2, 2, Color(0, 77, 230, 255));
	...
	Render::Outline(window->m_x, window->m_y, window->m_iWidth, window->m_iHeight, Color(90, 90, 90, 255));
	...
	if (TabCount) // If there are some tabs
	{
		...
		{
			RECT TabArea = { window->m_x + 8 + (i*TabSize), window->m_y + 1 + 10, TabSize, 27 };
			CTab *tab = window->Tabs[i];
			if (window->SelectedTab == tab)
			{
				Render::GradientV(window->m_x + 8 + (i*TabSize), window->m_y + 1 + 11, TabSize, 28, Color(0, 77, 230, 255), Color(0, 77, 230, 255));
			}
			else if (IsMouseInRegion(TabArea))
			{
				Render::GradientV(window->m_x + 8 + (i*TabSize), window->m_y + 1 + 11, TabSize, 28, Color(0, 120, 230, 255), Color(0, 120, 230, 255));
			}
...
void CGUI::SaveWindowState(CWindow* window, std::string Filename)
{
	...
	tinyxml2::XMLElement *Root = Doc.NewElement("Retro");
	...
	{
		MessageBox(NULL, "Failed To Save Config File!", "Retro", MB_OK);
	}
